...
import { guestCartUtils, GuestCartItem } from "@/lib/guestCartUtils";
...
export default function GuestCheckoutForm() {
    ...
    const fetchOptions = async () => {
      ...
      try {
        const res = await fetch(
          "https://alpa-be.onrender.com/api/cart/checkout-options"
        );
        const data = await res.json();

        if (data.success) {
          const methods: ShippingMethod[] = data.shippingMethods || [];
          const gstOptions: GSTOption[] = data.gstOptions || [];

          setShippingMethods(methods);
          if (methods.length > 0) setSelectedShippingId(methods[0].id);

          const defaultGST = gstOptions.find((g) => g.isDefault) || gstOptions[0];
          if (defaultGST) setGstId(defaultGST.id);
        }
      } catch (err) {
  ...
  // Recalculate totals whenever cart, shipping, or coupon changes
  const recalculateTotals = useCallback(async () => {
    if (cartItems.length === 0 || !selectedShippingId || !gstId) return;
    ...
    try {
      ...
      const res = await fetch(
        "https://alpa-be.onrender.com/api/cart/calculate-guest",
        {
          method: "POST",
      ...
      if (data.success) {
        setLiveTotals({
          ...
          gstPercentage: data.gstPercentage || "10.00",
  ...
  }, [cartItems, selectedShippingId, gstId]);
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const grandTotal = parseFloat(liveTotals?.grandTotal || "0");
      const res = await fetch(
        "https://alpa-be.onrender.com/api/coupons/validate",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ code, cartTotal: grandTotal }),
        }
      );
      const data = await res.json();
      if (data.success) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!selectedShippingId) errors.shippingMethodId = "Please select a shipping method.";
  ...
  const handleContinueToPayment = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "https://alpa-be.onrender.com/api/payments/guest/create-intent",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
      ...
      if (!res.ok || !data.success) {
        ...
        // Handle specific errors per doc
      ...
      // Store in sessionStorage for 3DS redirect fallback
  ...
  if (cartItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <Link
          href="/shop"
          className="px-6 py-3 bg-[#5A1E12] text-white rounded-lg font-medium hover:bg-[#441208] transition"
        >
          Continue Shopping
        </Link>
      </div>
      ...
      <div className="flex-1 px-4 md:px-8 lg:px-12 pt-20 pb-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 text-sm text-[#5A1E12]/60 mb-4">
            <span className={currentStep === "form" ? "font-bold text-[#5A1E12]" : ""}>
              1. Your Details
            </span>
            <ChevronRight className="w-4 h-4" />
            <span className={currentStep === "payment" ? "font-bold text-[#5A1E12]" : ""}>
              2. Payment
            </span>
          </div>
          ...
          <p className="text-sm mt-2 text-[#5A1E12]/70">
            Have an account?{" "}
            <Link href="/login" className="underline font-semibold text-[#5A1E12] hover:text-[#441208]">
              Log in
            </Link>{" "}
            or{" "}
            <Link href="/signup" className="underline font-semibold text-[#5A1E12] hover:text-[#441208]">
              Create Account
            </Link>
          </p>
        </div>
          ...
          <div className="space-y-6 bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-white border rounded-lg focus:outline-none focus:ring-1 focus:ring-[#5A1E12] transition-all text-sm ${
                      fieldErrors.customerName ? "border-red-400" : "border-[#5A1E12]/20"
                    }`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-white border rounded-lg focus:outline-none focus:ring-1 focus:ring-[#5A1E12] transition-all text-sm ${
                      fieldErrors.customerPhone ? "border-red-400" : "border-[#5A1E12]/20"
                    }`}
                  />
              ...
              <div className="mt-4">
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 bg-white border rounded-lg focus:outline-none focus:ring-1 focus:ring-[#5A1E12] transition-all text-sm ${
                    fieldErrors.customerEmail ? "border-red-400" : "border-[#5A1E12]/20"
                  }`}
                />
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {fieldErrors.addressLine && (
                    <p className="mt-1 text-xs text-red-500">{fieldErrors.addressLine}</p>
                  )}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Sydney"
                      className={`w-full px-4 py-3 bg-white border rounded-lg focus:outline-none focus:ring-1 focus:ring-[#5A1E12] transition-all text-sm ${
                        fieldErrors.city ? "border-red-400" : "border-[#5A1E12]/20"
                      }`}
                    />
                    {fieldErrors.city && (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="NSW"
                      className={`w-full px-4 py-3 bg-white border rounded-lg focus:outline-none focus:ring-1 focus:ring-[#5A1E12] transition-all text-sm ${
                        fieldErrors.state ? "border-red-400" : "border-[#5A1E12]/20"
                      }`}
                    />
                    {fieldErrors.state && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="2000"
                      className={`w-full px-4 py-3 bg-white border rounded-lg focus:outline-none focus:ring-1 focus:ring-[#5A1E12] transition-all text-sm ${
                        fieldErrors.zipCode ? "border-red-400" : "border-[#5A1E12]/20"
                      }`}
                    />
                    {fieldErrors.zipCode && (
                      <p className="mt-1 text-xs text-red-500">{fieldErrors.zipCode}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#5A1E12] mb-1">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-white border border-[#5A1E12]/20 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#5A1E12] transition-all text-sm"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="border-t border-[#5A1E12]/10 pt-6">
              ...
              {fieldErrors.shippingMethodId && (
                <p className="mb-2 text-xs text-red-500">{fieldErrors.shippingMethodId}</p>
              )}
                    ...
                    <div className="flex-1">
                      ...
                      {method.estimatedDays && (
                        <p className="text-xs text-[#5A1E12]/60">
                          Est. {method.estimatedDays} business days
                        </p>
                      )}
                    </div>
              ...
              {appliedCoupon ? (
                <div className="flex items-center gap-2 px-4 py-2.5 bg-green-50 border border-green-300 rounded-lg">
                  ...
                  <span className="flex-1 text-sm font-medium text-green-700">
                    {appliedCoupon.code} — −${appliedCoupon.discount.toFixed(2)} discount applied
                  </span>
                  <button
                ...
                <div key={item.productId} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500">
                      Qty {item.quantity} × ${parseFloat(String(item.product?.price || 0)).toFixed(2)}
                    </p>
                  </div>
                  <p className="font-medium text-sm">
                    ${(item.quantity * parseFloat(String(item.product?.price || 0))).toFixed(2)}
                  </p>
                </div>
              ))}
            </div>
            ...
            {loadingTotals ? (
              ...
              <>
                <div className="flex justify-between text-sm">
                  ...
                  <span>${parseFloat(liveTotals.subtotal).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Shipping</span>
                  <span>
                    {parseFloat(liveTotals.shippingCost) === 0
                      ? "Free"
                      : `$${parseFloat(liveTotals.shippingCost).toFixed(2)}`}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">
                    GST (incl. {parseFloat(liveTotals.gstPercentage).toFixed(1)}%)
                  </span>
                  <span>${parseFloat(liveTotals.gstAmount).toFixed(2)}</span>
                </div>
                {appliedCoupon && (
